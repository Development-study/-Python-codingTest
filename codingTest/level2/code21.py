...
def solution(k, tangerine):
    ...
    tangerine_dictionary = {key: 0 for key in tangerine}

    for t in tangerine:
        tangerine_dictionary[t] += 1
    
    tangerine_dictionary = dict(sorted(tangerine_dictionary.items(), key=lambda item : -item[1]))

    size = 0
    for value in tangerine_dictionary.values():
        if size >= k:
            break
        else:
            size += value
